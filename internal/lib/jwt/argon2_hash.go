...
func PasswordHashArgon2(password string, p settings.PasswordHashArgon2Params, salt, pepper []byte) string {
	...
	hashStr := fmt.Sprintf(
		...
		string(settings.PasswordHashArgon2),
		argon2.Version,
		p.Memory,
		p.Time,
		p.Parallelism,
...
func PasswordMatchArgon2(hashStr, password string, pepper []byte) (bool, error) {
	...
	p := settings.PasswordHashArgon2Params{}
	parts := strings.Split(hashStr, "$")
	if len(parts) != 6 {
		return false, fmt.Errorf("%s: invalid argon hash string: %s", method, hashStr)
	}
	...
	if version != argon2.Version {
		return false, fmt.Errorf("%s: unsupported Argon2 version: %d", method, version)
	}
