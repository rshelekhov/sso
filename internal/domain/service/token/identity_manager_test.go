...
func TestTokenService_ExtractUserIDFromContext(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "Success",
			setupContext: func() context.Context {
				return context.WithValue(context.Background(), "Token", createSignedToken(t, privateKey, jwt.MapClaims{
		...
		},
		{
			name: "Invalid token",
			setupContext: func() context.Context {
				return context.WithValue(context.Background(), "Token", "invalid-token")
			},
